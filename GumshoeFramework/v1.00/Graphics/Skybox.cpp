...
void Skybox::Initialize(ID3D11Device* device_)
{
    ...
    // Load the shaders
    const std::wstring shaderPath = GumshoeFrameworkDir() + L"Shaders\\Skybox.hlsl";
        ...
        for(uint64 s = 0; s < 6; ++s)
        {
            for(uint64 y = 0; y < CubeMapRes; ++y)
            {
                for(uint64 x = 0; x < CubeMapRes; ++x)
                {
                    ...
                    texels[idx] = Float4(radiance, 1.0f);
                }
            }
        }
...
static void MapPolarToTexel(float phi, float theta, uint32 width, uint32 height,
                            uint32& outX, uint32& outY)
{
    float percentX = phi / (2.0f * Pi);
    float percentY = theta / (Pi / 2.0f);

    if(percentX < 0.0f)
        percentX = 1.0f + percentX;
    if(percentY < 0.0f)
        percentY = 1.0f + percentY;

    outX = uint32(percentX * width);
    outY = uint32(percentY * height);
...
static Float3 MapPolarToXYZ(float phi, float theta)
{
    float sx = sin(theta) * sin(phi);
    float sy = cos(theta);
    float sz = sin(theta) * cos(phi);

    return Float3::Normalize(Float3(sx, sy, sz));
}

static void MapXYZToPolar(Float3 XYZ, float& outPhi, float& outTheta)
{
    outTheta = acos(XYZ.y);
    outPhi = atan2(XYZ.x, XYZ.z);
}

void Skybox::Save2DSkyAsEXR(ID3D11Device* device, const SkyCache& skyCache,
                            uint32 width, uint32 height, Float3 sunColor,
                            float sunSize, bool32 includeSun, bool32 normalizeIntensity,
                            const wchar* filePath)
{
    TextureData<Float4> textureData;
    textureData.Init(width, height, 1);

    float maxRadiance = 1.0f;
    float cosSunAngularRadius = std::cos(DegToRad(sunSize));
    ...
    if(normalizeIntensity)
    {
        for(uint32 y = 0; y < height; ++y)
        {
            ...
            {
                float xPercent = (float(x) / width);
                float yPercent = (float(y) / height);

                float phi = xPercent * (2.0f * Pi);
                float theta = yPercent * (Pi / 2.0f);

                Float3 sampleDir = MapPolarToXYZ(phi, theta);

                Float3 sampleRadiance = SampleSky(skyCache, sampleDir);
                if(Float3::Length(sampleRadiance) > maxRadiance)
                    maxRadiance = Float3::Length(sampleRadiance);
            }
        }
    ...
    const float invMaxRadiance = 1.0f / maxRadiance;

    for(uint32 y = 0; y < height; ++y)
    {
        for(uint32 x = 0; x < width; ++x)
        {
            float phi = (float(x) / width) * (2.0f * Pi);
            float theta = (float(y) / height) * (Pi / 2.0f);

            float sx = std::sin(theta) * std::sin(phi);
            float sy = std::cos(theta);
            float sz = std::sin(theta) * std::cos(phi);
            Float3 sampleDir = Float3::Normalize(Float3(sx, sy, sz));

            Float3 sampleRadiance = SampleSky(skyCache, sampleDir);
            sampleRadiance *= invMaxRadiance;
            if(includeSun)
            {
                float cosGamma = Float3::Dot(sampleDir, skyCache.SunDirection);
                if(cosGamma >= cosSunAngularRadius)
                    sampleRadiance += sunColor;
            }

            sampleRadiance = Float3::Clamp(sampleRadiance, 0.0f, FP16Max);

            uint32 idx = y * width + x;
            textureData.Texels[idx] = Float4(sampleRadiance, 1.0f);
        }
    }

    /*if(includeSun)
    {
        // Add sun in to the sky
        float sunTheta, sunPhi;
        MapXYZToPolar(skyCache.SunDirection, sunPhi, sunTheta);

        // Find bounding box of sun
        uint32 minX, minY;
        uint32 maxX, maxY;
        float sunRadius = DegToRad(sunSize);
        MapPolarToTexel(sunPhi - sunRadius, sunTheta - sunRadius, width, height, minX, minY);
        MapPolarToTexel(sunPhi + sunRadius, sunTheta + sunRadius, width, height, maxX, maxY);
        if(minX > maxX)
            Swap(minX, maxX);
        if(minY > maxY)
            Swap(minY, maxY);

        // Super sample all the texels to find out if we are within sun extents
        for(uint32 y = minY; y <= maxY; ++y)
        {
            for(uint32 x = minX; x <= maxX; ++x)
            {
                ...
                for(uint32 sY = 0; sY <= sampleCount; ++sY)
                {
                    for(uint32 sX = 0; sX <= sampleCount; ++sX)
                    {
                        float texWidthInRad = (1.0f / width) * (2.0f * Pi);
                        float texHeightInRad = (1.0f / height) * (Pi / 2.0f);
                        float sPhi = phi + (float(sX) / sampleCount) * texWidthInRad;
                        float sTheta = theta + (float(sY) / sampleCount) * texHeightInRad;

                        Float3 dir = MapPolarToXYZ(sPhi, sTheta);

                        float angleBetween = Clamp(Float3::Dot(dir, skyCache.SunDirection), -1.0f, 1.0f);
                        if(angleBetween <= sunRadius)
                            ++hitCount;
                    }
                }
                ...
                float percent = float(hitCount) / (sampleCount * sampleCount);
                Float3 sampleRadiance = SampleSky(skyCache, sampleDir);
                Float3 sunRadiance = sunColor;
                Float3 result = sampleRadiance + sunRadiance;
                result *= invMaxRadiance;

                // Clamp result to max supported fp16 value
                result = Float3::Clamp(result, 0.0f, FP16Max);

                uint32 idx = y * width + x;
                textureData.Texels[idx] = Float4(result, 1.0f);
            }
        }
    }*/
