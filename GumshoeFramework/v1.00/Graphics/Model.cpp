...
// Initializes the mesh as a cornea
...
/*
// Initializes the mesh as a sphere
void Mesh::InitSphere(ID3D11Device* device, const float& diameter, const Float3& position,
                      const uint64& tessellation, uint32 materialIdx)
{
    const float radius = diameter/2.0f;
    
    if (tessellation < 3)
        throw std::out_of_range("tessellation parameter out of range for sphere model generation");

    float radius = diameter / 2.0f;
    const uint64 verticalSegments = tessellation;
    const uint64 horizontalSegments = tessellation * 2;

    const uint64 NumVerts = (verticalSegments+1) * (horizontalSegments+1);
    const uint64 NumTriangles = (verticalSegments) * (horizontalSegments+1) * 2;
    const uint64 NumIndices = NumTriangles * 3;
    
    std::vector<Vertex> sphereVerts(NumVerts);
    std::vector<uint32> sphereIndices(NumIndices, 0);
    uint64 vIdx = 0;
    uint64 iIdx = 0;

    // Create rings of vertices at progressively higher latitudes.
    for (uint64 i = 0; i <= verticalSegments; i++)
    {
        float v = 1 - (float)i / (float)verticalSegments;

        float latitude = (i * Pi / verticalSegments) - Pi_2;
        float dy, dxz;

        XMScalarSinCos(&dy, &dxz, latitude);

        // Create a single ring of vertices at this latitude.
        for (uint64 j = 0; j <= horizontalSegments; j++)
        {
            float u = (float)j / (float)horizontalSegments;

            float longitude = (float)j * Pi2 / (float)horizontalSegments;
            float dx, dz;
            ...
            Vertex& vertex   = sphereVerts[vIdx];
    ...
    uint64 stride = horizontalSegments + 1;
    ...
    for (uint64 i = 0; i < verticalSegments; i++)
    {
        ...
        {
            uint64 nextI = i + 1;
            uint64 nextJ = (j + 1) % stride;

            sphereIndices[iIdx++] = (i * stride + j);
            sphereIndices[iIdx++] = (nextI * stride + j);
            sphereIndices[iIdx++] = (i * stride + nextJ);

            sphereIndices[iIdx++] = (i * stride + nextJ);
            sphereIndices[iIdx++] = (nextI * stride + j);
            sphereIndices[iIdx++] = (nextI * stride + nextJ);
        }
    }
                      ...
                      const uint64& tessellation, uint32 materialIdx)
{
    ...
    for (uint64 i = 0; i <= verticalSegments; i++)
    {
        ...
        for (uint64 j = 0; j <= horizontalSegments; j++)
        {
            ...
            Vertex& vertex   = sphereVerts[vIdx];
            vertex.Position  = normal * radius + position;
            vertex.Normal    = normal;
            vertex.Tangent   = Float3();
            vertex.Bitangent = Float3();
            vertex.TexCoord  = Float2(u, v);
    ...
    // Fill the index buffer with triangles joining each pair of latitude rings.
    uint32 stride = (uint32)horizontalSegments + 1;
    ...
    const uint32 vbSize = vertexStride * numVertices;
    vertices.resize(vbSize, 0);
    memcpy(vertices.data(), sphereVerts.data(), vbSize);

    const uint32 ibSize = indexSize * numIndices;
    indices.resize(ibSize, 0);
    memcpy(indices.data(), sphereIndices.data(), ibSize);
...
void Model::GenerateRandomTestScene(ID3D11Device* device, const Float2& dimensions, const Float3& position)
{
    MeshMaterial material;
    material.DiffuseMapName = L"ancient_flooring.jpg";
    material.NormalMapName = L"ancient_flooring_n.jpg";
    fileDirectory = L"..\\Content\\Textures\\";
    LoadMaterialResources(material, fileDirectory, device, false);
    meshMaterials.push_back(material);

    // Create a scene with a floor, 2 random boxes, and a random sphere, placing all models on the floor
    meshes.resize(4);
    meshes[0].InitPlane(device, dimensions, position, Quaternion(), 0);
    meshes[1].InitBox(device, Float3(2.0f), Float3(0.0f, 2.0f/2.0f, 1.5f), Quaternion(), 0);
    meshes[2].InitBox(device, Float3(2.0f, 5.0f, 2.0f), Float3(6.0f, 5.0f/2.0f, 6.5f), Quaternion(), 0);
    meshes[3].InitSphere(device, 2.0f, Float3(-2.0f, 2.0f, -2.0f), 100, 0);
}
